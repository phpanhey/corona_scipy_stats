...
def calculatePreviousDayInfections(infected_data):
    return infected_data


def getPreAndPostLockdownFragments(infected_data):
    lockdown = dict(
        itertools.islice(
            infected_data.items(), list(infected_data.keys()).index("22.03.2020")
        )
    )

    no_lockdown = {k: v for k, v in infected_data.items() if k not in lockdown}
...
infected_data = addMissingInfections(infected_data)
infected_data = calculatePreviousDayInfections(infected_data)
no_lockdown, lockdown = getPreAndPostLockdownFragments(infected_data)




# print(json.dumps(no_lockdown, indent=4))
